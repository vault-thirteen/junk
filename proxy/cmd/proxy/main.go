...
func handler(w http.ResponseWriter, req *http.Request) {
	...
	log.Debug().Msgf("time taken to serve '%v' is %v", req.URL.String(), time.Since(t1).String())
}

func processHttpRequest(w http.ResponseWriter, req *http.Request) {
	log.Debug().Msgf("http request to '%s'", req.URL.String())
...
func processHttpsRequest(w http.ResponseWriter, req *http.Request) {
	log.Debug().Msgf("https request to '%s'", req.URL.String())
	...
	if !ok {
		...
		log.Error().Msg("hijacking is not supported")
		return
	}
