...
func (c *Controller) MoveForumDown(params *json.RawMessage, _ *jrm1.ResponseMetaData) (result any, re *jrm1.RpcError) {
	var p *rm.MoveForumDownParams
	re = jrm1.ParseParameters(params, &p)
	if re != nil {
		return nil, re
	}

	var r *rm.MoveForumDownResult
	r, re = c.moveForumDown(p)
	if re != nil {
		return nil, re
	}
...
func (c *Controller) moveForumDown(p *rm.MoveForumDownParams) (result *rm.MoveForumDownResult, re *jrm1.RpcError) {
	// Get caller user.
	var user *cm.User
	...
	err := dbC.MoveForumDown(p.Forum)
	if err != nil {
		return nil, c.databaseError(err)
	}

	result = &rm.MoveForumDownResult{
